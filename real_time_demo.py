"""
Features:
  - Video trackbar (scrub frames).
  - Press SPACE or 'p' to toggle play/pause.
  - Press 'c' for chat (if --chat).
  - Segmentation as colored outlines.
  - Phase + PCO risk displayed in top-left corner.
  - Technique feedback printed in the terminal.
...
# A global list of instrument names that correspond to class IDs 0..12
# Must match your segmentation classes in EXACT order.
INSTRUMENT_NAMES = [
    "Background",
    "Iris",
    "Pupil",
...
def find_contours_for_class(seg_mask, class_id):
    """
    Return list of contours for the specified class_id in seg_mask.
    """
...
def draw_segmentation_outlines(bgr_img, seg_mask):
    """
    Draw outlines for each class on bgr_img in the corresponding color from INSTRUMENT_COLORS.
    """
    for cid in np.unique(seg_mask):
        if cid == 0:
            continue
        c = INSTRUMENT_COLORS[cid]
        contours = find_contours_for_class(seg_mask, cid)
        cv2.drawContours(bgr_img, contours, -1, c, thickness=2)
    return bgr_img

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--video", type=str, default=None, help="Path to .mp4 or camera index (default=0).")
    parser.add_argument("--seg_model", type=str, default="lightweight_seg.pth", help="Path to segmentation model .pth")
    parser.add_argument("--phase_model", type=str, default="phase_recognition.pth", help="Path to phase model .pth")
    parser.add_argument("--chat", action="store_true", help="Enable chat mode in terminal.")
    ...
    if device.type == "cuda":
        print("Mixed precision is available for faster inference.")
    ...
    # 2) Load phase model
    #   We'll assume 12 phases. Adjust if needed.
    ...
    print(f"Total frames in this video: {total_frames if total_frames>0 else 'Unknown or 0'}")

    cv2.namedWindow("SurgeryDemo", cv2.WINDOW_NORMAL)
    if total_frames > 0:
        cv2.createTrackbar("Frame", "SurgeryDemo", 0, total_frames-1, lambda x: None)
    ...
    print("'c'         : Chat mode (terminal)")
    print("'q'         : Quit program")
    print("Trackbar    : Jump frames in video (when paused)")
    print("====================")

    while True:
        if playing:
            # if playing, let the capture read next frame
            current_frame_index = int(cap.get(cv2.CAP_PROP_POS_FRAMES))
        else:
            # if paused, jump to trackbar
            if total_frames > 0:
                tb_pos = cv2.getTrackbarPos("Frame", "SurgeryDemo")
                current_frame_index = tb_pos
                cap.set(cv2.CAP_PROP_POS_FRAMES, current_frame_index)

        ret, frame_bgr = cap.read()
        if not ret:
            # If we reach end or error
            if total_frames > 0:
                # wrap around
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                current_frame_index = 0
            else:
                print("End of camera stream or read error.")
                break
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=(device.type=="cuda")):
            ...
            seg_out_dict = seg_model(seg_in)  # {'out':..., 'aux':...} if aux_loss=True
        ...
        # technique feedback -> print in terminal
        ...
        # Display top-left text (phase + PCO)
        ...
        if key == ord('q'):
            ...
            # chat in terminal
            query = input("Chat (type 'done' to exit): ")
            while query.lower() != "done":
                resp = assistant.answer(query)
